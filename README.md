...
* при модификации пользователя (временно: нет взаимодействия с flexo коллекциями):
    * осуществляется изменения в mongo посредством flexo коллекции
    * создается модель пользователя
...
* для получения шаблона раздела:
    ...
    * пересекаем их с разрешениями по роли, и по пользователю (пересекаем разрешения для отображаемых элементов, а так же проверяем разрешения на отображения разделов в ссылках)
    * если в модели раздела указаны view, то:
